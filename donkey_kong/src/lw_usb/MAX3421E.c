...
//
// 	xil_printf("lmao xd\n");
//	int status = XSpi_SetOptions(&SpiInstance, XSP_CLK_PHASE_1_OPTION | XSP_CLK_ACTIVE_LOW_OPTION);
//
//	if (status != XST_SUCCESS){
//		xil_printf("lmao xd\n");
//		return XST_FAILURE;
//	}
	...
	int ret = XSpi_Transfer(&SpiInstance, wdata, rdata, 2);
	if (ret)
		xil_printf("error writing maxreg byte: %d\n", ret);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int ret = XSpi_Transfer(&SpiInstance, wdata, rdata, nbytes+1);
	if (ret)
		xil_printf("error writing maxreg bytes: %d\n", ret);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int ret = XSpi_Transfer(&SpiInstance, wdata, rdata, 2);
	if (ret)
		xil_printf("error reading maxreg: %d\n", ret);
